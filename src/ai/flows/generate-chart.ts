  ...
  async (input) => {
    ...
    try {
      ...
      if (output) {
        ...
        return RecommendedVisualizationSchema.parse(output);
      }
      return null;
    } catch (error) {
        console.error("Error generating chart or validating schema:", error);
        // If the model output is malformed or an error occurs, return null.
        return null;
    }
  }
);
